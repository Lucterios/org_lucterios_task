...
// --- Last modification: Date 10 November 2011 5:53:47 By  ---
...
if ($SECURITY_LOCK->isLock()==0) {
	...
	while($task_list->fetch()) {
		if ($lastProject!=$task_list->projet) {
			$lastProject=$task_list->projet;
			if ($task_list->projet>0) {
				$DBProject=$task_list->getField('projet');
				$lab=new Xfer_Comp_LabelForm('ProjectTitle'.$t.'-'.$DBProject->id);
				$lab->setLocation($posX,$posY++,4);
				$lab->setValue('{[newline]}{[hr/]}{[center]}{[bold]}{[italic]}'.str_replace('$','&#160;',str_pad($DBProject->nom,20,'$')).'{[italic]}{[/bold]}'.$DBProject->getValue().'{[/center]}{[hr/]}');
				$xfer_result->addComponent($lab);
			}
		}
